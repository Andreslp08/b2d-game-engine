import { GameAssetsTypes, type AssetToPreload } from "engine/common/interfaces/assets";

export const PLAYER_SKIN2_ATLAS_ASSETS:  AssetToPreload<any>[] = [
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-idle",
		path: "/assets/atlas/player/skin2/player-idle.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-idle-no-arms",
		path: "/assets/atlas/player/skin2/player-idle-no-arms.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-run",
		path: "/assets/atlas/player/skin2/player-run.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-run-no-arm",
		path: "/assets/atlas/player/skin2/player-run-no-arm.json",
	},

	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-jump",
		path: "/assets/atlas/player/skin2/player-jump.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-jump-no-arm",
		path: "/assets/atlas/player/skin2/player-jump-no-arm.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-idle",
		path: "/assets/atlas/player/skin2/player-idle.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-idle-no-arms",
		path: "/assets/atlas/player/skin2/player-idle-no-arms.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-run",
		path: "/assets/atlas/player/skin2/player-run.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-run-no-arm",
		path: "/assets/atlas/player/skin2/player-run-no-arm.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-jump",
		path: "/assets/atlas/player/skin2/player-jump.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-jump-no-arm",
		path: "/assets/atlas/player/skin2/player-jump-no-arm.json",
	},
	{
		type: GameAssetsTypes.Atlas,
		name: "atlas:player-skin2-dash",
		path: "/assets/atlas/player/skin2/player-dash.json",
	},
];
